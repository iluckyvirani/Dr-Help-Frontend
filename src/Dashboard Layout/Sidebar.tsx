          ...
          {menuItems.map((item) => (
            <NavLink
              ...
              className={`flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-200 group relative ${
                isActive(item.path)
                  ? 'bg-linear-to-r from-blue-600/20 to-cyan-600/20 text-white'
                  : 'text-slate-400 hover:text-white hover:bg-slate-800/50'
              }`}
            >
              {/* Active Indicator */}
              {isActive(item.path) && (
                <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-linear-to-b from-blue-500 to-cyan-400 rounded-r-full" />
              )}
              
              <span className={`${isActive(item.path) ? 'text-cyan-400' : 'group-hover:text-cyan-400'} transition-colors`}>
                {item.icon}
              </span>
              ...
              {/* {item.badge && !isCollapsed && (
                <span className="ml-auto px-2 py-0.5 text-xs font-semibold bg-linear-to-r from-blue-500 to-cyan-500 text-white rounded-full">
                  {item.badge}11
                </span>
              )} */}
